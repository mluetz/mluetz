...
UI = [
    ...
    ('fillSelect(document.getElementById("fStatus"),"Alle Status",distinct("status").sort());',
     'fillSelect(document.getElementById("fStatus"),"All statuses",distinct("status").sort());'),
    ...
    ('const open=FINDINGS.filter(f=>/offen/i.test(f.status)).length;',
    ...
    ('tile(open,"Status: Offen","k-open",open/t,"var(--ink-3)");',
     'tile(open,"Status: Open","k-open",open/t,"var(--ink-3)");'),
    # CSV
